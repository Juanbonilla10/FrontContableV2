...
document.addEventListener("DOMContentLoaded", function () {
  ...
  $("#saveCard").click(function () {
    ...
    if (body.cardNumber != "" & body.bank != "" & body.value != "" & body.franchise != "") {
       console.log("Si hay datos para guardar la card")
       saveCard(body);
    } else {
      Swal.fire({
        title: "Error",
        text: "Datos incompletos para crear la tarjeta",
        icon: "error",
      });
    }
  });
  ...
  function saveCard(data) {
    requestBody = {
        idcards: 0,
        card_number: `000000000000${data.cardNumber}`,
        banks_id: data.bank,
        value: data.value,
    ...
    $.ajax({
      method: "POST",
      url: `${urlEndPoint}/Cards/saveCard`,
      dataType: "json",
      headers: {
        Authorization: "Bearer " + token,
      },
      ...
      complete: function (response) {
        if (response.status == 201) {
          Swal.fire({
            title: "Guardado!",
            text: "El tipo de tarjeta ha sido creado",
            icon: "success",
          });
  ...
  function getCards() {
    $.ajax({
      method: "GET",
      url: `${urlEndPoint}/Cards/allCardsByUser`,
      dataType: "json",
      headers: {
        Authorization: "Bearer " + token,
      },
      ...
      complete: function (response) {
        if (response.status == 200) {
          console.log("Obteniendo datos getCards");
          mapListCardsUser(JSON.parse(response.responseText));
          mapListCards(JSON.parse(response.responseText));
        } else {
  ...
  deleteCard = function (card) {
    ...
    if(card != ""){
        $.ajax({
            method: "DELETE",
            url: `${urlEndPoint}/Cards/deleteCard/${card}`,
            dataType: "json",
            headers: {
              Authorization: "Bearer " + token,
            },
            ...
            complete: function (response) {
              if (response.status == 200) {
                Swal.fire({
                  title: "Eliminado!",
                  text: "La tarjeta ha sido borrada",
                  icon: "success",
                });
                getCards();
              } else {
                Swal.fire({
                  title: "Lo sentimos!",
                  text: "La tarjeta no ha sido borrada",
                  icon: "error",
                });
              }
            },
  ...
  function mapListCardsUser(data) {
    let cadena = "";
      ...
      cadena += `
            ...
            <div class="d-flex flex-column">
                ...
                <span class="text-dark font-weight-bold ms-sm-2"
                    >${element.franchisesDescription}</span
                ></span
                >
                <span class="mb-2 text-xs"
                >Ultimos digitos de la tarjeta:
                <span class="text-dark ms-sm-2 font-weight-bold">
                    ${element.card_number}</span
                ></span
                >
                <span class="text-xs"
                >Estado:
            ...
            <div class="ms-auto text-end">
                <a onclick="deleteCard(${element.idcards})"
    ...
    $("#listCardsUser").html(cadena);
  }

  function mapListCards(data){
                  ...
                  <div class="col-md-6 col-6">
                    <div class="card">
                      ...
                      <div class="card-body pt-0 p-3 text-center">
                        ...
                        <h5 class="mb-0">$${element.limit_amount}</h5>
                      </div>
                    </div>
                  </div>
                  <div class="col-md-6 col-6">
                    <div class="card">
                      ...
                      <div class="card-body pt-0 p-3 text-center">
                        ...
                        <hr class="horizontal dark my-3" />
                        <h5 class="mb-0">$${element.value}</h5>
                      </div>
                    </div>
                  </div>
                </div>
